...
nlp = spacy.load("/models/ru_core_news_sm")
with open('geo.yml', 'r') as f:
  data = safe_load(f)

COUNT = 3

embedding_function = SentenceTransformerEmbeddings(model_name="/models/USER-bge-m3")
cross_model = CrossEncoder('/models/cross-encoder-russian-msmarco', max_length=512)
...
def process_text(user_input, places):
    ...
    - Коротко напиши интересный факт или шутку о том, что ищет клиент.
    - Коротко напиши обзор о заведениях на основе отзывов.
    Заведения начинаются с 'Название'.
    Если клиент не написал ничего, что относится к поиску ресторана или еде, то ответь '-'.
    Если в отзывах нет данных, соответствующих запросу, предложи альтернативную еду на основе отзывов.
    Отзывы: {places}
    """ 
